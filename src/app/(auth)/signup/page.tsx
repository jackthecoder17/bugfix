  ...
  return (
    <div className="w-full flex flex-col gap-3">
      <div className="flex flex-col md:flex-row gap-3">
        <TextInput errRef={fullNameErrRef} placeholder="e.g. John" label="Full Name" onChange={(e) => handleOnChange({ fullName: e.target.value })} value={formState.fullName} name={"fullName"}/>
        <TextInput errRef={emailErrRef} placeholder="e.g. mail@domain.com" label="Email address" onChange={(e) => handleOnChange({ email: e.target.value })} value={formState.email} name={"email"}/>
      </div>
      <TextInput errRef={usernameErrRef} placeholder="e.g. john23" label="Username" onChange={(e) => handleOnChange({ username: e.target.value })} value={formState.username} name={"username"} />
      <TextInput errRef={passwordErrRef} placeholder="e.g. 2insksdioi23" label="Password" inputType="password" onChange={(e) => handleOnChange({ password: e.target.value })} value={formState.password} name={"password"} />
      <TextInput errRef={accessCodeErrRef} placeholder="e.g. 2insksdioi23" label="Access Code" inputType="password" onChange={(e) => handleOnChange({ accessCode: e.target.value })} value={formState.accessCode} name={"accessCode"} />
      <button type="button" onClick={handleFormSubmit} className="mt-2 bg-blue text-white rounded-md capitalize p-2 w-full outline-none" disabled={isLoading}>
        { isLoading ? (
